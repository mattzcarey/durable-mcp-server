/**
 * SSE-aware JSON-RPC helpers for driving the worker's /mcp surface: real
 * Streamable HTTP POSTs via SELF. The modern (2026-07-28) builder threads the
 * per-request `_meta` envelope and the SEP-2243 routing headers (`Mcp-Method`,
 * `Mcp-Name`) a native client sends; the legacy builder keeps the app's
 * original claim-less request shape. Adapted from the durable-mcp-server
 * package's test/support/jsonrpc.ts.
 */
...
export const MCP_URL = "http://example.com/mcp";
...
export interface ModernRequestOptions {
   ...
   * `params.name` (tools/call), `params.taskId` (tasks/*), or `params.uri`
   * (resources/read).
   */
  mcpName?: string | null;
}
...
export function modernRequest(
  ...
  const derivedName = [params["name"], params["taskId"], params["uri"]].find(
...
/** Builds a legacy (2025-era, claim-less) JSON-RPC request. */
...
export async function postLegacy(
  ...
  return SELF.fetch(legacyRequest(method, params, id));
}
